...
# This will suppress INFO and WARNING messages.
...
# Disable oneDNN optimizations to avoid potential performance issues on some systems
...
master_logs_DIR = PROJECT_DIR / "logs" / "master_pipeline" / f"{datetime_str}"
if not master_logs_DIR.exists():
    ...
    # master_logs_DIR.mkdir(exist_ok=True)

# get the dataset directory for storing or accessing data files
Hubeau_dataset_DIR = PROJECT_DIR / "dataset" / "Hubeau"

# Import the main training functions for each forecasting model
from src.models.prophet.train_prophet import main_train_prophet
from src.models.xgboost.train_xgboost import main_train_xgboost
from src.models.lstm.train_lstm_tensorflow import main_train_lstm
...
def main():
    ...
    with mlflow.start_run(run_name=f"pipeline_station_{code_station}") as run:
        ...
        fig.write_html(str(master_logs_DIR / f"master_comparison_plot_{site_name}_{code_station}.html"))
        mlflow.log_artifact(str(master_logs_DIR / f"master_comparison_plot_{site_name}_{code_station}.html"), artifact_path="Plots")
        
        # Temporarily save the raw & cleaned dataset to CSV for artifact logging
        start_date_str = raw_df["date_obs_elab"].min() # get the minimum date of the observations
        end_date_str = raw_df["date_obs_elab"].max() # get the maximum date of the observations
        start_date_str = start_date_str.replace(" ", "") # formatage date pour nom fichier
        end_date_str = end_date_str.replace(" ", "") # formatage
        
        # Save the raw fetched data to a CSV file for reference and debugging
        raw_path_format = f"raw_{datetime_str}_hubeau_obs_elab_{site_name}_{code_site}_{code_station}_{start_date_str}_{end_date_str}"
        raw_file = str(Hubeau_dataset_DIR / "01_raw" / f"{raw_path_format}.csv")
        raw_df.to_csv(raw_file, index=False)
        
        # save the cleaned data to a CSV file for reference and debugging
        cleaned_path_format = f"cleaned_{datetime_str}_hubeau_obs_elab_{site_name}_{code_site}_{code_station}_{start_date_str}_{end_date_str}"
        cleaned_file = str(Hubeau_dataset_DIR / "02_processed" / f"{cleaned_path_format}.csv")
        clean_df.to_csv(cleaned_file, index=False)
        
        # Log the dataset artifact to MLflow to ensure complete reproducibility
        mlflow.log_artifact(raw_file, artifact_path="Data")
        mlflow.log_artifact(cleaned_file, artifact_path="Data")
