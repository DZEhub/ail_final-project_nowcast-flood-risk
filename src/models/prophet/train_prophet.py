...
# Get logs directory to store artifacts and plots
# Get project directory
PROJECT_DIR = Path(__file__).resolve().parent.parent.parent.parent
# Create log directory if not exist to store artifacts and plots
datetime_str = datetime.now().strftime("%Y%m%d_%H%M%S")
prophet_logs_DIR = PROJECT_DIR / "logs" / "prophet" / datetime_str
if not prophet_logs_DIR.exists():
    ...
    prophet_logs_DIR.mkdir(parents=True, exist_ok=True)
    # prophet_logs_DIR.mkdir(exist_ok=True)
...
def main_train_prophet(train_df: pd.DataFrame, test_df: pd.DataFrame, 
    ...
    with mlflow.start_run(run_name=f"prophet_station_{code_station}") as run:
        ...
        prophet_fig.write_html(str(prophet_logs_DIR / f"prophet01_forecast_plot_{code_station}.html"))
        ...
        fig3.savefig(str(prophet_logs_DIR / f"prophet02_components_plot_{code_station}.png"))
        
        # Log the figures to MLflow as an HTML artifact for dashboard visualization
        mlflow.log_artifact(str(prophet_logs_DIR))
